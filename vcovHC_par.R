vcovHC_par <- function(x, ...) {
  UseMethod("vcovHC_par")
}

vcovHC_par.default <- function(x, 
                           type = c("HC3", "const", "HC", "HC0", "HC1", "HC2", "HC4", "HC4m", "HC5"),
                           omega = NULL, sandwich = TRUE,cl=NULL, ...)
{
  type <- match.arg(type)
  rval <- meatHC_par(x, type = type, omega = omega, cl=cl)
                   ...
                   omega = NULL, cl=NULL)
{
  ...
  #diaghat <- try(hatvalues.ivreg_par(x), silent = TRUE)
  diaghat <- hatvalues.ivreg_par(x, cl=cl)
