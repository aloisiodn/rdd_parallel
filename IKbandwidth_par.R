...
IKbandwidth.par <-function (X,Y,cutpoint=NULL,verbose=FALSE,kernel="triangular") {
  ...
  h2_side <-c(3.56*(Nl^(-1/7))*(varY/(fbarx*max(m3^2,0.01)))^(1/7) , 3.56*(Nr^(-1/7))*(varY/(fbarx*max(m3^2,0.01)))^(1/7))
  
  
  side <- rbind((X>=(cutpoint-h2_side[1])) & (X<cutpoint) , (X>=cutpoint) & (X<= (cutpoint+h2_side[2])))
  
  left<-(X>=(cutpoint-h2_side[1])) & (X<cutpoint)
  left2<-side[1,]

  f<-foreach(i = 1:2, .combine = rbind) %dopar% {
    
    #New bandwidth estimate left
    
    N_side <- sum(side[i,])
    if(N_side==0)
      stop("Insufficient data in vicinity of the cutpoint to calculate bandwidth.")
    
    mod<-lm(Y~poly(cX,2,raw=T),subset=side[i,])
    
    m2_side<-2*coef(mod)[3]
    r_side<-720*varY/(N_side*(h2_side[i]^4))
    
    c(m2=m2_side, r=r_side)
  }  
  ...
  optbw<-ck*(2*varY/(fbarx*((f[2,1]-f[1,1])^2+f[2,2]+f[1,2])))^(1/5)*(Nx^(-1/5))
