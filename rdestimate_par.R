...
RDestimate.par<-function(formula, data, subset=NULL, cutpoint=NULL, bw=NULL, kernel="triangular", se.type="HC1", cluster=NULL, verbose=FALSE, model=FALSE, frame=FALSE) {
  call<-match.call()
  ...
  if(is.null(bw)) {
    print(paste("par bw time:", system.time(bw<-IKbandwidth.par(X=X,Y=Y,cutpoint=cutpoint,kernel=kernel, verbose=verbose))[3]))
    bws<-c(bw,.5*bw,2*bw)
  ...
  Tr<-as.integer(X>=0)
  #####################################################################################
  ff<-foreach(ibw = 1:length(bws)) %dopar% {
    bw<-bws[ibw]
    e_o <- new.env()
    ...
    if(type=="sharp"){
      ...
      if (is.null(cluster)) {
        #
        e_o$se <-coeftest(mod,vcovHC(mod,type=se.type))[2,2]
      } else {
        #
        e_o$se <-robust.se(mod,cluster[na.ok][w>0])
      }
      #
      e_o$z<-e_o$est/e_o$se
      e_o$p<-2*pnorm(abs(e_o$z),lower.tail=F)
      e_o$ci<-c(e_o$est-qnorm(.975)*e_o$se,e_o$est+qnorm(.975)*e_o$se)
      #
      if(model) e_o$model=mod
      if(frame) e_o$frame=dat.out
      ...
      #m_ff <-foreach (f_model = 1:2) %dopar% {
      #
      #  if (f_model == 1) {
      #    mod <- lm(form1,weights=w,data=subset(data,w>0))
      #  } else {
      ...
      if (is.null(cluster)) {
        #########################################################
        print("par vcovHC start")
        t_ = system.time(debug_vcovHC <- vcovHC_par(mod,type=se.type, cl=cl))[3]
        print(paste("par vcovHC time:",t_))
        e_o$se<-coeftest(mod,debug_vcovHC)[2,2]
        #########################################################
      } else {
        e_o$se<-robust.se(mod,cluster[na.ok][w>0])
      }
      e_o$z<-e_o$est/e_o$se
      e_o$p<-2*pnorm(abs(e_o$z),lower.tail=F)
      e_o$ci1<-e_o$est-qnorm(.975)*e_o$se
      e_o$ci2<-e_o$est+qnorm(.975)*e_o$se
      
      e_o$model<-list()
      e_o$frame<-list()
      if(model) {
        e_o$model$firststage<-mod1
        e_o$model$iv=mod
      }
      if(frame) e_o$frame=dat.out
    }
  ...
  o$type<-type
  o$call<-call
  o$est<-as.vector(c(ff[[1]]$est, ff[[2]]$est, ff[[3]]$est),mode="numeric")
  names(o$est)<-names(bws)
  o$bw<-as.vector(bws)
  o$se<-c(ff[[1]]$se, ff[[2]]$se, ff[[3]]$se)
  o$z<-c(ff[[1]]$z, ff[[2]]$z, ff[[3]]$z)
  o$p<-c(ff[[1]]$p, ff[[2]]$p, ff[[3]]$p)
  o$obs<-c(ff[[1]]$obs, ff[[2]]$obs, ff[[3]]$obs)
  ...
  o$model<-list()
  if(type=="fuzzy") {
    o$model$firststage<-list(ff[[1]]$firststage,ff[[2]]$firststage,ff[[3]]$firststage)
    o$model$iv<-list(ff[[1]]$iv,ff[[2]]$iv,ff[[3]]$iv)
  }
  o$frame<-list(ff[[1]]$frame,ff[[2]]$frame,ff[[3]]$frame)
